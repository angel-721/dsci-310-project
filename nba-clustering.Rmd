...
### Fit a k-means model
```{r echo = FALSE}
kmModel <- kmeans(nba_stats_final, 3)
...
nba_stats_final <- nba_stats_processed |> select(-Pos)
# First, define the feature columns using tidyselect helpers
feature_columns <- c( "DRB.", "STL.", "BLK.", "DWS", "DBPM")
# Then create the training data using these columns
nba_stats_final <- nba_stats_processed |> 
  select( all_of(feature_columns))
...
cluster_position_summary <- nba_stats_processed %>%
  group_by(cluster, Pos) %>%
  summarise(count = n()) %>%
  group_by(cluster) %>%
  mutate(proportion = count / sum(count) * 100)

ggplot(cluster_position_summary, aes(x = cluster, y = count, fill = Pos)) +
